...
class Funcionario extends Pessoa
{
    ...
    public static function search($nome)
    {
        $sql = new Sql();
        return $sql->select(
            "SELECT f.*, (SELECT p.* from pessoa p WHERE p.nome LIKE :SEARCH ORDER BY p.idPessoa )  
    ...
    public function loadUserById($idFuncionario)
    {
        ...
        $sql = new Sql();
        $idP = $sql->select(
            "SELECT pessoa_idpessoa FROM funcionario WHERE idFuncionario = :ID",
            array(
    ...
    public function login($usuario, $senha)
    {
        $sql = new Sql();
        $results = $sql->select(
            "SELECT f.* (SELECT p.* from pessoa p WHERE p.usuario = :LOGIN and p.senha = :PASSWORD)
                                    FROM funcionario f WHERE f.pessoa_idpessoa = p.idPessoa;",
            array(
                ":LOGIN" => $usuario,
                ":PASSWORD" => $senha
    ...
    public function insert()
    {
        ...
        $sql->query(
            "INSERT INTO funcionario (pessoa_idpessoa, cargo_idcargo, horario) 
                                value (:PESSOA, :CARGO, :HORARIO)",
            array(
                ":PESSOA" => $idPessoa[0]["idPessoa"],
                ":CARGO" => '1',
                ":HORARIO" => $this->getHorario()
            )
        );
